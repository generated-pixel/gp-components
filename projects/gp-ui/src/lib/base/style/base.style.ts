...
export abstract class BaseStyle<TTheme extends ComponentStyleConfig = ComponentStyleConfig> {
  ...
  private applyHostStyles(
    host: HTMLElement,
    renderer: Renderer2,
    hostStyles?: Record<string, StyleValue>,
  ): void {
  ...
  private applyVars(
    ...
    const entries = Object.entries(variables).map(
      ([key, value]) => [this.toCssVarName(key), value] as const,
    );
    const nextKeys = new Set(entries.map(([key]) => key));
...
export function cloneComponentVars(
  vars?: ComponentVarsDefinition,
): ComponentVarsDefinition | undefined {
  if (!vars) {
    return undefined;
...
export function denormalizeComponentVars(
  vars: NormalizedComponentVars,
): ComponentVarsDefinition | undefined {
