...
export class ThemeSwitcher extends Base {
  ...
  override onInit(): void {
    ...
    const targetId = this.pickExistingThemeId([configThemeId, providedInitialId], options) ?? options[0]?.id
    ...
    const modeTarget =
      this.pickExistingThemeMode([configThemeMode, providedInitialMode], modeOptions) ?? modeOptions[0]?.id ?? 'auto'
  ...
  private pickExistingThemeId(ids: Array<string | undefined>, options: ResolvedThemeOption[]): string | undefined {
    for (const id of ids) {
      if (!id) {
        continue
      }
