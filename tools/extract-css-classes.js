...
function main() {
  ...
  walkDirectory(options.srcDir, (filePath) => {
    ...
    } else if (filePath.endsWith('.ts') && !filePath.endsWith('.spec.ts') && !filePath.endsWith('.stories.ts')) {
  ...
  console.log(`Generated ${classes.length} utilities at ${path.relative(process.cwd(), options.outputFile)}`)
...
function collectClassesFromBoundAttribute(binding, classStore) {
  ...
  if (binding.type === BindingType.Property && (bindingName === 'class' || bindingName === 'ngClass')) {
...
function extractClassesFromAngularExpression(ast) {
  ...
  if (Array.isArray(ast.keys) && Array.isArray(ast.values)) {
    return ast.keys.map((entry) => (entry && typeof entry.key === 'string' ? entry.key : null)).filter(Boolean)
  }
  ...
  if (Array.isArray(ast.properties)) {
    return ast.properties.flatMap((prop) => extractClassesFromAngularExpression(prop.value || prop.expression))
  }
...
function handleStringLiteral(node, classStore) {
  ...
  if (ts.isCallExpression(parent)) {
    ...
    if (CLASS_LIST_METHODS.has(calleeInfo.property) && calleeInfo.qualifier.endsWith('.classList')) {
      classStore.add(text)
      return
    }
    ...
    if (ARRAY_MUTATION_METHODS.has(calleeInfo.property) && looksLikeCssClassesQualifier(calleeInfo.qualifier)) {
      classStore.add(text)
      return
    }
  ...
  if (ts.isBinaryExpression(parent) && parent.operatorToken.kind === ts.SyntaxKind.EqualsToken) {
    if (ts.isPropertyAccessExpression(parent.left) && parent.left.name && parent.left.name.text === 'className') {
      text
        .split(/\s+/)
        .filter(Boolean)
        .forEach((item) => classStore.add(item))
...
function isWithinComponentHostObject(propertyAssignment) {
  ...
  if (ts.isCallExpression(parent) && ts.isIdentifier(parent.expression) && parent.expression.text === 'Component') {
    return true
  }
...
function translateGridUtility(base) {
  ...
  if (colSpanMatch) {
    ...
    if (!Number.isNaN(span)) {
      return {
        declarations: [{ prop: 'grid-column', value: `span ${span} / span ${span}` }],
      }
    }
  }
  ...
  if (rowSpanMatch) {
    ...
    if (!Number.isNaN(span)) {
      return {
        declarations: [{ prop: 'grid-row', value: `span ${span} / span ${span}` }],
      }
    }
  }
...
function translateTypographyUtility(base) {
  ...
  if (fontMatch) {
    ...
    if (FONT_WEIGHT_SCALE[token]) {
      return {
        declarations: [{ prop: 'font-weight', value: FONT_WEIGHT_SCALE[token] }],
      }
    }
    if (FONT_FAMILY_SCALE[token]) {
      return {
        declarations: [{ prop: 'font-family', value: FONT_FAMILY_SCALE[token] }],
      }
    }
  }
  ...
  if (leadingMatch) {
    ...
    if (LINE_HEIGHT_SCALE[token]) {
      return {
        declarations: [{ prop: 'line-height', value: LINE_HEIGHT_SCALE[token] }],
      }
    }
    const arbitrary = parseArbitraryValue(token)
    if (arbitrary) {
  ...
  if (trackingMatch) {
    ...
    if (LETTER_SPACING_SCALE[token]) {
      return {
        declarations: [{ prop: 'letter-spacing', value: LETTER_SPACING_SCALE[token] }],
      }
    }
    const arbitrary = parseArbitraryValue(token)
    if (arbitrary) {
...
function translateBorderUtility(base) {
  if (base === 'border') {
    return {
      declarations: [{ prop: 'border-width', value: BORDER_WIDTH_SCALE.DEFAULT }],
    }
  }
